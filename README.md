...
2. A coordinator splits the file inventory it into partitions.  Each
...
### Complete Workflow Sequencing
Most projects will require both backfill and forward processing to create a
complete, regularly updated Icechunk store.  To reduce properly sequence commits
and reduce merge conflicts the optimal approach is to

1. Deploy your pipeline with `BACKFILL_ENABLED` set to `True` and your Icechunk
