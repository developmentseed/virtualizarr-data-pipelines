...
concatenate archival files without having to think too much about architecture
code.
...
Virtualizarr Data Pipelines uses a strongly-typed [settings file](./cdk/settings.py) that allows you to configure things like bucket names and external SNS topics used by the CDK infrastructure when you deploy it.  Many of the settings include defaults but you can also specify and override values with a `.env` file.  A [sample file](./.env.sample) is provided as an example.
